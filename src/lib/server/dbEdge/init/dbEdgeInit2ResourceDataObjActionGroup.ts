import { sectionHeader } from '$server/dbEdge/init/dbEdgeInitUtilities1'
import { addDataObjActionFieldGroup } from '$server/dbEdge/init/dbEdgeInitUtilities2'

export async function initPreDataObjActionGroups() {
	sectionHeader('initPreDataObjActionGroups')
	await initFeatures()
}

/* auth */
async function initFeatures() {
	await addDataObjActionFieldGroup({
		actionFieldItems: [['noa_save_detail', 0]],
		name: 'doag_auth_my_account',
		owner: 'app_sys'
	})

	/* base */
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_detail', 0],
			['noa_save_cancel', 1],
			['noa_new_detail', 2],
			['noa_delete_detail', 3]
		],
		name: 'doag_detail',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_list', 0],
			['noa_new_detail_list', 1],
			['noa_edit_list', 2],
			['noa_reorder', 3],
			['noa_reorder_cancel', 4]
		],
		name: 'doag_list',
		owner: 'app_sys'
	})

	await addDataObjActionFieldGroup({
		actionFieldItems: [['noa_save_list', 0]],
		name: 'doag_list_edit',
		owner: 'app_sys'
	})

	/* embed */
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_detail', 0],
			['noa_save_cancel', 1]
		],
		name: 'doag_embed_detail',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_list', 0],
			['noa_new_detail_embed_config_list', 1],
			['noa_edit_list_embed_config', 2],
			['noa_reorder_embed_config_list', 3],
			['noa_reorder_cancel', 4]
		],
		name: 'doag_embed_list_config',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [['noa_edit_list_embed_select', 0]],
		name: 'doag_embed_list_select',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_list', 0],
			['noa_reorder_embed_config_list', 1],
			['noa_reorder_cancel', 2]
		],
		name: 'doag_embed_list_edit',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_list', 0],
			['noa_edit_list_embed_edit_parm_value', 1]
		],
		name: 'doag_embed_list_edit_parm_value',
		owner: 'app_sys'
	})

	/* dialog */
	await addDataObjActionFieldGroup({
		actionFieldItems: [['noa_dialog_done', 0]],
		name: 'doag_dialog_footer_detail',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_dialog_cancel', 0],
			['noa_dialog_done', 1]
		],
		name: 'doag_dialog_footer_list',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_detail', 0],
			['noa_save_cancel', 1],
			['noa_new_detail_dialog_detail', 2],
			['noa_delete_detail', 3]
		],
		name: 'doag_dialog_form_detail',
		owner: 'app_sys'
	})

	/* report */
	await addDataObjActionFieldGroup({
		actionFieldItems: [['noa_refresh_list', 0]],
		name: 'doag_report_render',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_detail', 0],
			['noa_save_cancel', 1]
		],
		name: 'doag_detail_report',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_list', 0],
			['noa_edit_list', 1],
			['noa_reorder', 2],
			['noa_reorder_cancel', 3]
		],
		name: 'doag_list_report',
		owner: 'app_sys'
	})

	/* specialty */
	await addDataObjActionFieldGroup({
		actionFieldItems: [
			['noa_save_detail', 0],
			['noa_save_cancel', 1],
			['noa_new_detail', 2],
			['noa_delete_detail', 3],
			['noa_migrate', 4]
		],
		name: 'doag_detail_migrate_define',
		owner: 'app_sys'
	})
	await addDataObjActionFieldGroup({
		actionFieldItems: [['noa_process_execute', 0]],
		name: 'doag_detail_migrate_process',
		owner: 'app_sys'
	})
}
